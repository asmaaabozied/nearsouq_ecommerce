...
class OrderController extends Controller
{
    ...
    public function canceledOrder(Request $request)
    {
        ...
        if (isset($request->order_id)) {
        ...
        }elseif(isset($request->order_detail_id)){
            $orderdetails = OrderDetail::where('id', $request->order_detail_id)->first();
            $order = Order::find($orderdetails->order_id);
        }
        
            $orderdetails = OrderDetail::where('order_id', $request->order_id)->orWhere('id', $request->order_detail_id)->get();
            if($operation === 'canceled'){
                ...
                foreach($orderdetails as $order_detail){
                        ...
                        if($order->payment_status === 'NOT_PAID'){
                            $order_detail->update(['status' => 'CANCELLED_ACCEPTED', 'reason_id' => $request->reason_id]);
                            ApiHelper::addToOrderHistory($order_detail->order_id, $order_detail->id, $order_detail->shop_id, $order_detail->product_id, 'CANCELLED_ACCEPTED', Auth::id());
                        }elseif($order->payment_status === 'PAID'){
                            $order_detail->update(['status' => 'CANCELLED_ACCEPTED', 'reason_id' => $request->reason_id]);
                            ApiHelper::addToWallet(Auth::id(),$order_detail->discount_price ? $order_detail->discount_price : $order_detail->price,'site.messages.return_order_value', NULL, $order_detail->id );
                            ApiHelper::addToOrderHistory($order_detail->order_id, $order_detail->id, $order_detail->shop_id, $order_detail->product_id, 'CANCELLED_ACCEPTED', Auth::id());
                ...
                return $this->responseJson(1, _('site.messages.no_products_in_the_order'));
            }

            }elseif($operation === 'returned'){
                if(!empty($orderdetails)){
                foreach($orderdetails as $order_detail){
                    if($order_detail->status === 'APPROVED_BY_CAPTAIN'|| $order_detail->status === 'SHIPPED' || $order_detail->status === 'DELIVERED'){
                        $order_detail->update(['status' => 'RETURNED_ACCEPTED']);
                        ApiHelper::addToWallet(Auth::id(),$order_detail->discount_price ? $order_detail->discount_price : $order_detail->price,'site.messages.return_order_value', NULL, $order_detail->id );
                        ApiHelper::addToOrderHistory($order_detail->order_id, $order_detail->id, $order_detail->shop_id, $order_detail->product_id, 'RETURNED_ACCEPTED', Auth::id());
            ...
            if($order->payment_status === 'PAID'&& ($order_detail->status === 'READY'|| $order_detail->status === 'RECEIVED')){
                ApiHelper::addToWallet(Auth::id(),$order->delivery_cost,'site.messages.return_delivery_value', $order_detail->id, NULL );
            }
    ...
    public function confirmOrder(Request $request)
    {
        ...
        if (isset($request->order_id)) {
            $order = Order::find($request->order_id);
             // dd($order);
            if($request->operation === 'WALLET'){
                $user_wallet = Wallet::where('user_id',Auth::id())->first();
                if($user_wallet){
                if($user_wallet->balance >= $order->total){
                    $user_wallet->update(['balance'=> $user_wallet->balance - $order->total]);
                    $wallet_log = new Walletlog();

                    $wallet_log->amount = $order->total;
                    $wallet_log->user_id = Auth::id();
                    $wallet_log->payment_method = 7;
                    $wallet_log->comment = trans('site.messages.WITHDRAW_From_Wallet'). $request->order_id;
                    $wallet_log->operation = 'WITHDRAW';
                    ...
                    foreach($orderdetails as $orderdetail){
                        ApiHelper::createCustomerTrans($orderdetail->shop_id, $orderdetail->order_id, $orderdetail->id, NULL, $order->user_id ? $order->user_id : NULL, 'WALLET', 'ORDER', $orderdetail->discount_price ? $orderdetail->discount_price : $orderdetail->price);
                    }
                }else{
                    return response()->json(['status' => 1, 'message' => __('site.messages.wallet_balance_is_not_enough')]);
                }
                }else{
                    return response()->json(['status' => 1, 'message' => __('site.messages.you_do_not_have_wallet_record')]);
                }
            }else{
                $order->update(['confirmed' => 'TRUE',
                'payment_type' => "CASH",
...
// return $this->responseJson(1, __($distance."++++ cost=".$delivery_cost));
        ...
        //check items exists in shop
        $fav = ApiHelper::totalCart();
        if($fav['IN_FAVORITE'] === "IN_FAVORITE")
        {
            ...
            foreach ($items as $product) {
                ...
                if ($check_products->quantity >= $qty) {
                    ...
                    if ($product_data->discount_price > 0) {
                        ...
                        $commsion_without_vat = $amount_without_vat * (0 + $shop->commission / 100);
                    } else {
                        $product_total_price = (($product_data->price + $product_data->variants()) * $qty);
                        $amount_without_vat = $product_total_price / (1 + $shop->vat / 100);
                        $commsion_without_vat = $amount_without_vat * (0 + $shop->commission / 100);
                    }
                    $commission_with_vat = $commsion_without_vat * 1.15;//=====must get from backend
                    $commission = $shop->commission;
                    // $commission_value = $amount_without_vat - $amount_without_commsion + $commission_vat;
                    $vat = $shop->vat;
                    $vat_value = $product_total_price - $amount_without_vat;

                    
                    $merchant_will_get_with_vat = $product_total_price - $commission_with_vat;
                    ...
                    $orders = OrderDetail::create([
                        ...
                        'shop_id' => $shopId,
                        'commsion' => $commission,
                        'commsion_value' => $commission_with_vat,
                        'merchant_will_get' => $merchant_will_get_with_vat,
...
// if variants request save into database
                    ...
                    ApiHelper::addToOrderHistory($order->id, $orders->id, $shopId, $productId, 'RECEIVED', Auth::id());

                    $user = User::where('shop_id',$shopId)->first();
                    if($user != NULL){
                    $user_device = Device::where('user_id',$user->id)->where('platform','WEB')->first();
                    // dd($user);
                     if($user_device != NULL){
                     $client = $user_device;
                     //dd($client);
                     if($client){
                     if ($client->one_signal_id != null) {
                         $text = 'site.messages.new_order_noti';
                         $title = 'site.new_order';
                ...
                if(in_array($order_detail->shop_id, $shop_ids)){
                    foreach($orders_for_one_shop as $order_for_one_shop){
                        if($order_for_one_shop->shop_id === $order_detail->shop_id){
                            ...
                            $order_for_one_shop->merchant_will_get += $order_detail->merchant_will_get;
                            $order_for_one_shop->commsion_value += $order_detail->commsion_value;
                        }
                    }
                }else{
           ...
           /*foreach($orders_for_one_shop as $order_for_one_shop){
               ...
               //dd($order_for_one_shop->commsion_value);
               $transaction->debit += $order_for_one_shop->merchant_will_get;
               $transaction->name_id = 1;
               $transaction->final_balance += $order_for_one_shop->merchant_will_get ;
           ...
           if($request->delivery == 1){
            $operating_cost = Setting::where('param','operating_cost')->value('value');
            $delivery_cost= Deliverycalculator::where('user_id',Auth::id())->sum('cost')+$operating_cost;
            ...
            //     if (!$carts->isEmpty()) {
                ...
                $today = Carbon::today();
                $offer = DeliveryOffer::where('start_date','<=',$today)
                ->where('end_date','>=',$today)
                ->where('status','enable')
                ->first();
                if($offer){
                if($offer->total_cart <= $total && !$offer->total_delivery){
                    $data['total_offer'] = $delivery_cost * $offer->discount_percentage / 100;
                    $delivery_cost = $delivery_cost - $data['total_offer'];
                    $delivery_offer = $offer->discount_percentage;
                }elseif($offer->total_delivery <= $delivery_cost && !$offer->total_cart){
                    $data['total_offer'] = $delivery_cost * $offer->discount_percentage / 100;
                    $delivery_cost = $delivery_cost - $data['total_offer'];
                    $delivery_offer = $offer->discount_percentage;
                }
...
// update into table order to total
            ...
            $order->update(['total' => $total, 'subtotal' => $subtotal, 'delivery_cost' => $delivery_cost, 'delivery_offer' => $delivery_offer, 'delivery_distance_in_km' => $distance]);
            ...
            /*foreach($shop_ids as $shop_id){
                $pdf1Path = 'uploads/invoices/ordercommsion.'.$dataForPdf1['id'].'.shop'. $shop_id .'.pdf';
                $dataForPdf1['seller'] = Shop::where('id',$shop_id)->first();
                //dd($dataForPdf1['shop']);
                $dataForPdf1['shop_orders'] = [];
                $vatcommsionfororder = 0;
                $vatwithcommsionfororder = 0;
                $dataForPdf1['totalcommsion'] =0 ;
                $dataForPdf1['totalvat'] = 0;
                $dataForPdf1['totalvatwithcommsion'] = 0;
                foreach($ordersForPdf as $order){
                    if($order->shop_id == $shop_id){
                        array_push($dataForPdf1['shop_orders'], $order);
                    }
                }

                foreach($dataForPdf1['shop_orders'] as $order){
                    $order['vatcommsionfororder'] = $order->commsion_value * 15 / 100;
                    $order['vatwithcommsionfororder'] = $order['vatcommsionfororder']  + $order->commsion_value;
                    $dataForPdf1['totalcommsion'] += $order->commsion_value;
                    $dataForPdf1['totalvat'] += $order['vatcommsionfororder'];
                    $dataForPdf1['totalvatwithcommsion'] += $order['vatwithcommsionfororder'];
                }

                PDF::loadView('pdf.commsion', $dataForPdf1, [], [
                    'title' => 'Another Title',
                    'margin_top' => 0,
    ...
    public function TotalDeliveryCost(Request $request)
    {
        ...
        if (!empty($delivery_calculators)) {
            $total_cost = Setting::where('param','operating_cost')->value('value');
    ...
    public function addCart(Request $request)
    {
        ...
        foreach ($request['products'] as $product) {
            ...
            */
            ...
            $old_quantity = Cart::where('product_id',$productId)->where('shop_id',$shopId)->where('user_id',Auth::id())->sum('quantity');
            $cost = DeliveryCost::where('min_distance', '<=', $distance)->where('max_distance', '>=', $distance)->first();
            if ($cost) {
                if ($shopProduct && $shopProduct->quantity >= $qty+$old_quantity) {
...
// if variants in request store in database
        ...
        foreach ($ordersArray as $value) {
            ...
            if ($shopchecked) {
                $dataDelivery = Deliverycalculator::updateOrCreate(['shop_id' => $value->shop_id, 'user_id' => $value->user_id], ['shop_id' => $value->shop_id, 'user_id' => $value->user_id,
                    'latitude' => $request->latitude, 'longitude' => $request->longitude, 'cost' => ($distance * $cost->price + $operating_cost),
    ...
    public function showCart()
    {
        ...
        if ($carts->isEmpty()) {
            ...
            $total_cart =  $array['total'];
            if($array['IN_FAVORITE'] != NULL){
                $deleteSomeProducts = true;
            }else{
                $deleteSomeProducts = false;
            }
            $cart = CartResource::collection($array['carts']);

            $data = array("total" => $array['total'], 'subtotal' => $array['subtotal']);

            return response()->json(['status' => 1, 'cart' => $data, 'cart_details' => $cart, 'deleteSomeProducts'=>$deleteSomeProducts]);
        }
    }
}
